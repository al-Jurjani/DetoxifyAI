...
def evaluate_prompt_strategy(
    strategy_name: str, eval_data: list, embedding_model
) -> dict:
    """
...
def main():
    ...
    parser.add_argument(
        "--eval-file", default="experiments/eval.jsonl", help="Path to eval.jsonl"
    )
    parser.add_argument(
        "--limit", type=int, default=10, help="Number of samples for CI (default: 10)"
    )
    parser.add_argument(
        "--strategies",
        nargs="+",
    ...
    parser.add_argument(
        "--output", default="experiments/ci_eval_results.json", help="Output file"
    )
